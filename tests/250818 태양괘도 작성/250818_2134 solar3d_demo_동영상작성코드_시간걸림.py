...
# 시작 각도 (2025년 개략 위치, degree → radian 변환)
earth_start = np.deg2rad(0.0)      # 지구
...
mars = np.array([mars_radius * np.cos(2*np.pi*t/mars_period + mars_start),
                 ...
                 0.05 * np.sin(2*np.pi*t/mars_period + mars_start)])  # 약간 기울기
...
ax.set_title("3D Orbits of Earth, Mars, and Jupiter (approx. 2025 start)")
ax.set_xlabel("X [AU]")
ax.set_ylabel("Y [AU]")
ax.set_zlabel("Z [AU]")
...
mars_line, = ax.plot([], [], [], color="orange", lw=1, label="Mars orbit")
...
ani.save("tests/solar3d_earth_mars_jupiter.mp4", writer=writer)

print("✅ 동영상 저장 완료: tests/solar3d_earth_mars_jupiter.mp4")
